...
While the implementation above works, it introduces 2 new problems:

- Reliance to JavaScript, even though `<textarea>` works fine without it
- Need to listen to various other event such as `change`, `paste`, `compositionend`, and more depending on your needs.
Failing to handle events might cause the `<textarea>` to not autosize itself and possibly hurting UX in the process.

> If you're controlling the `<textarea>` with front-end libraries such as React, Vue, Svelte, etc, always
> remember to call the autosizing function or synthesize native events after changing the `<textarea>` value
> programmaticaly. For example:
>

Adding to the list of unsupported CSS features, `::before`, `::after`, and [CSS Highlighting API]() also don't
