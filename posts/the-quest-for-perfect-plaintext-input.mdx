...
- Developers need to wire the autosizing logic manually to other HTML events such  as `paste` and direct value modification through element reference with `.innerHTML`, depending on the need.
Failing to handle events might cause the `<textarea>` to not autosize itself and hurting UX in the process.
