...
However, rich-text formatting applied through `.innerHTML` still applies to the element. In layman terms, `plaintext-only` is a
...
it covers all basic `<textarea>` capabilities while supporting CSS features `<textarea>` doesn't support especially with the addition of `plaintext-only`.

However, playing around `contenteditable` with `plaintext-only` a bit longer reveals the 'smells' that `<textarea>` doesn't have:

- Form submission requires JavaScript
- `contenteditable` attribute doesn't transform the containing element into a form control element,
which means no native support for `<label>` and `placeholder`
- Implementing placeholder for `contenteditable` requires extra care as browsers will always insert an extra `<br />`
after user have interacted with with `contenteditable` to preserve its visibility as a block-level element
...
So we now have 2 elements that can represents a multi-line plaintext input, both have their own advantages and weaknesses

 but which one is the perfect fit for the job?
