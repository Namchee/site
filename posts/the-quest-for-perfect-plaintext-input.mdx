...
Since we're currently dealing with multiline **plaintext** input, we shouldn't forget to drop the rich-text formatting
...
contenteditable.addEventListener('paste', (e) => {
  ...
  // insert text to current caret position
  document.execCommand('insertText', false, text);
});
```

Moreover, we also need to remember to ban direct value modification through element reference using
`.innerHTML` as it might open up XSS vulnerabilities.

Since we still need to do some legwork in JavaScript, using `contenteditable` is still prone to human errors. Personally,
I think `<div contenteditable>` doesn't offer substantial DX improvements over `<textarea>` for plaintext inputs.
