...
Since we still need to do some legwork in JavaScript, using `contenteditable` is still prone to human errors.
Unfortunately, we still need to find the middleground between `<textarea>` and `<div contenteditable>` for plaintext inputs.
