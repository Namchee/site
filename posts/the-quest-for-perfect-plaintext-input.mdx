...
![User agent Shadow DOM that encapsulate the contents of textarea](https://res.cloudinary.com/namchee/image/upload/v1748674003/posts/the-quest-for-perfect-plaintext-input/textarea-shadow-dom)

> User agent Shadow DOM are hidden by default in DevTools. To
...
Adding to the list of problems, the Shadow DOM encapsulation also causes

Adding to the list of unsupported CSS features, `::before`, `::after`, and
[CSS Highlighting API](https://developer.mozilla.org/en-US/docs/Web/API/CSS_Custom_Highlight_API) also don't
work at all with `<textarea>` without wizardy due to how the user agent handles `<textarea>`.
...
outlining that the main purpose of these new attributes is to extend the capability of `<textarea>` to do
rich-text formatting in general like WYSIWYG editors, not excluding plaintext multi-line inputs.

While `designMode` makes the *entire document* editable, `contenteditable` isolate the editability to a certain element which usually is a `<div>`.

<figure class="my-4 w-full max-w-4/5 mx-auto">
  <div contenteditable class="border border-separator p-2 font-mono w-full bg-background transition-colors min-h-16">Hello, I'm a contenteditable div 👋</div>
