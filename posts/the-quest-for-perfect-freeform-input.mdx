...
- Accessibility need to be handled manually through ARIA attributes, which usually aren't used
...
Those who avoid libraries and care about consistency will spend most of their time building a rendering model and mapping rules for
...
- Re-map all supported rich-text elements to your rendering model and strip the rest of unsupported elements. This method requires
more code.
...
However, `plaintext-only` doesn't mean that it strips the rich-text formatting capabilities from `contenteditable`. Rich-text
formatting is still supported as evidenced by rendering HTML elements inside `contenteditable` through `.innerHTML`.


doesn't disable the rich-text formatting capabilities as you can still apply rich-text formats with
`.innerHTML`.

Unfortunately, processing the clipboard into a plaintext is the only thing
