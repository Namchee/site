...
Arriving at this section, we now have 2 alternatives for freeform input elements: `<textarea>` and `contenteditable`. But how should you determine
which one to use?

Simply put, choosing between 2 alternatives for defining freeform inputs can be simplified using the following flowchart:
