...
- Inconsistent many-to-one mapping from from rich-text to HTML elements. For example, a **bold** text might be defined with `<b>` and `<strong>`
respectively.
- A block-level `contenteditable` always insert an extra `<br />` after interaction to preserve visibility which
breaks empty content detection without proper cleanup.
