...
import ChoosingYourPoison from '../src/assets/diagrams/posts/the-quest-for-perfect-freeform-input/choosing-your-own-poison.svg';
...
At this point, you might wonder which solution for freeform inputs fits your use case best. Fortunately, choosing between the alternatives can be simplified through the following diagram:

<ChoosingYourPoison />

Simply put, `contenteditable` is only worth it if you checked all of these:

1. You are dealing with a pretty complex rich-text contents.
2. You are working on a serious apps.
3. Existing solutions (pre-made libraries) don't fit your use cases, which is *very rare*.

> [!NOTE]
> You don't need a `contenteditable`-based solution if you only need a static highlighting with simple styling such as colors. Instead, stack a `textarea` with an element that supports styling such as a `<div>`. This trick has been used by some libraries such as [react-mentions](https://www.npmjs.com/package/react-mentions).
...
It is for this reason that I find the name `plaintext-only` a bit misleading since it doesn't transform `contenteditable` into a *plaintext-only* input, but it only provides a built-in handler for paste events.
