...
- All inputs within this directive must be [percent-encoded](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/encodeURIComponent)
	even for normally URL-safe characters such as dashes.
...
Things will go pretty simple if you're in full control of the content. All you have to do is to use [this polyfill](https://github.com/GoogleChromeLabs/text-fragments-polyfill) and
attach it to the URL using `encodeURIComponent`.

```js title=Building Text Fragment Programmatically
// Detect support
```
...
on stable content structure and mandatory element wrapping while enabling us to create a higher-quality citation and reference links.
