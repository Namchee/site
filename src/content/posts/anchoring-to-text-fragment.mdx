...
Imagine my joy when I found out that the feature is alive through a WICG proposal as it solves my major gripe with element fragments:

- Over-reliance on stable content structure
- Mandatory element wrapping to attach `id` to

At the same time, the text fragment is enabling us to create a higher-quality citation and reference links by sharing more specific text to focus on
instead of long and abstract sections.

With the advantages in mind, I hope that *you* will consider adopting text fragment directives as a higher-quality alternative for providing references compared to element fragments. Happy (deep)linking!
