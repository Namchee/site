...
Obviously, this is **not** what we should do! Replacing the default behavior of `<pre>` is a bad idea
in the long run as it is hard to replace pre-formatting in CSS. This means we can't just replace the existing
element via the `components` constant.

Since directly touching the code block element is a no-go, the safest way to customize the code block is by
*wrapping* it inside `<figure>` and customize the surrounding elements without
touching the `<pre>` block at all.

To wrap the `<pre>` element, we need a direct access to the `<pre>` block.
