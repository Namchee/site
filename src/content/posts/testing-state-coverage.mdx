...
ecosystem of the tech stack, we can easily test our code by writing the test suite in a testing framework and run
them through an automated test runner.
...
There are 2 main advantages of using code coverage as a metric for your tests:
...
<dl class=":uno space-y-6">
  ...
  <ListItem>
    ...
    is as simple as enabling a configuration, such as [Go](https://go.dev/doc/build-cover) through the `-cover` flag
    or by installing one additional dependencies in an opt-in way, such as [Vitest](https://vitest.dev/guide/coverage.html#coverage-setup)
    that installs [v8](https://v8.dev/blog/javascript-code-coverage) or [Istanbul](https://istanbul.js.org/) for coverage reporting.
  </ListItem>
  <ListTitle>
    Quantitative
  </ListTitle>
  <ListItem>
    Since code coverage is a quantitative metric, code coverage can be easily compared to one another. We can always say that a code
    with higher coverage should generally be more reliable than a code with low coverage as it covers more scenario.
  </ListItem>
</dl>
...
that has been executed by the test suite.

However, we can't say that code coverage measures test suites completely as it doesn't cover **states** of the program, which
is arguably the most important part of the program. While they sound similar, program state is not equivalent to statements.

To understand the pitfalls of code coverage in measuring program state, let's take a look again at function `divide` from the example above.
...
This particular function is tested by the following test case
...
able to tell us that the code has been tested throughoutly as all blocks of our code has been
covered by the suite. However, code coverage
