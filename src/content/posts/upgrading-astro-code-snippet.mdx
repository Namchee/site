...
> Since we *might* be passing more metadata in the future, it's advisable to define a rule schema for writing metadata to avoid
> problems during parsing in the future.
>
> <br />
>
> For the sake of simplicity, we are going to use `key1=value1,key2,...etc` that can be parsed with:
>
> ```astro
>	---
> const meta = {};
...
>
> if (props['data-meta']) {
>  attribs['data-meta'].split(',').forEach((prop) => {
...
To add code captions, we are going to pass `title` as a metadata to our component.
